...
def get_ibm(circuit):
    ...
    circuitos = []
    for i in circuit.json.keys():
        circuitos.append(ast.literal_eval(unquote(circuit.json[i]).split('circuit=')[1]))
...
def get_ibm_individual(ind_circuit,d):
...
def get_aws_individual(ind_circuit, d):
    url = ind_circuit
    circuitos = []
    if url:
        circuit = ast.literal_eval(unquote(url).split('circuit=')[1])
